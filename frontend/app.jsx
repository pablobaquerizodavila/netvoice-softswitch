...
const Logo = ({ size = 26 }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
    <img src="assets/mark-tight.png" alt="" width={size} height={size}
         style={{ display: "block" }} />
    <span style={{
      ...
      e<span className="brand-dot">.</span>neural
    </span>
  </div>
);

// ============================================================
// Boot Splash — first impression
// ============================================================

const Splash = () => {
  const [hide, setHide] = useState(false);
  const [phase, setPhase] = useState("Establishing session · SIP/TLS");

  useEffect(() => {
    const t1 = setTimeout(() => setPhase("Handshake complete · 11 regions online"), 1500);
    const t2 = setTimeout(() => setHide(true), 2600);
    const key = (e) => { if (e.key === "Escape" || e.key === "Enter") setHide(true); };
    window.addEventListener("keydown", key);
    return () => { clearTimeout(t1); clearTimeout(t2); window.removeEventListener("keydown", key); };
  }, []);

  return (
    <div className="splash" data-hide={hide}>
      <div className="splash-chrome">
        <span>console.e-neural.io</span>
      ...
      <div className="splash-mark">
        <img src="assets/mark-tight.png" alt="" />
      </div>
      <div className="splash-wordmark">
        e<span className="brand-dot">.</span>neural
      </div>
      <div className="splash-progress" />
      <div className="splash-status">{phase}</div>
      <button className="splash-skip" onClick={() => setHide(true)}>Skip →</button>
    </div>
  );
};
...
const Why = () => (
  <section className="section section-why" id="why">
    <div className="why-mark-wrap">
      <img src="assets/mark-pulse.png" alt="" />
    </div>
    <div className="container">
...
// ============================================================
const AccessPortals = () => (
  <section className="section" id="accesos" style={{background:'var(--bg-deep)'}}>
    <div className="container">
      <div className="section-head">
        <p className="mono small muted">— accesos</p>
        <h2 className="section-title">Portales del sistema</h2>
      </div>
      ...
      <div style={{marginTop:32,padding:'20px 24px',background:'var(--surface-1)',borderRadius:10,border:'1px solid var(--line)'}}>
        <p className="mono small muted" style={{margin:'0 0 12px'}}>— endpoints activos</p>
        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {[
            ['/v1/auth/login','Auth JWT'],
            ['/v1/onboarding/register','Registro'],
            ['/v1/contracts/sign','Contrato'],
            ['/v1/payments/init','Pago'],
            ['/v1/activation/activate/:id','Activación'],
            ['/v1/did/client/:id','DID'],
            ['/v1/partners','Partners'],
            ['/docs','API Docs'],
          ].map(([url,label]) => (
            ...
            >{url} <span style={{color:'var(--muted-2)'}}>— {label}</span></a>
          ))}
        </div>
      </div>
    </div>
...
const AccessCard = ({ tag, title, desc, href, color, primary }) => (
  <a href={href} style={{textDecoration:'none'}} className="platform-card">
    <div className="card-top">
      <span style={{
    ...
    <div className="card-title" style={{color:'var(--text)',marginTop:12}}>{title}</div>
    <div className="card-body" style={{color:'var(--text-2)',fontSize:14,lineHeight:1.6,marginTop:6}}>{desc}</div>
    <div style={{marginTop:14,display:'flex',alignItems:'center',gap:6,color:color,fontSize:13,fontWeight:500}}>
      Acceder <span style={{fontSize:16}}>→</span>
    </div>
  </a>
);
...
const Closing = () => (
  <section className="section section-closing" id="contact">
    <div className="container">
      <img className="closing-lockup" src="assets/lockup-horizontal.png" alt="e.neural" />
      <h2 className="closing-title">
        Tu red. <strong>Tu panel.</strong>
      </h2>
      <p className="closing-sub">
        Gestiona extensiones, trunks, CDR y métricas en tiempo real
        desde un panel construido sobre infraestructura
    ...
    <div className="container footer-inner">
      <div className="footer-brand">
        ...
        <p className="mono small muted">
          Infraestructura VoIP carrier-grade · © {new Date().getFullYear()} e.neural.
        </p>
      </div>
      <div className="footer-cols">
        <FooterCol title="Plataforma" links={["Softswitch", "SBC", "WebRTC", "Multi-tenant"]} />
        <FooterCol title="Panel" links={["Dashboard", "Extensiones", "CDR", "Métricas"]} />
        <FooterCol title="Acceso" links={["Panel admin", "Portal cliente", "Agente SAC", "API docs"]} hrefs={["/login", "/registro", "/sac/login", "/docs"]} />
      </div>
    </div>
  </footer>
);
